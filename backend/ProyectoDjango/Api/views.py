...
def check_user_exists(request):
    ...
    if not user or not password:
        return Response(
            {"error": "Username and cedula are required."},
    ...
    try:
        ...
        if check_password(password, userResponse.clave):
            ...
            return Response(
                {
                    "exists": True,
                    "message": f"User {user} authenticated.",
            ...
            return Response(
                {"exists": False, "message": "Password incorrect."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def create_pet(request):
    try:
        ...
        try:
            usuario_cliente = Usuarios.objects.get(
                usuario=usuario
            )  # Usar la columna usuario
