...
class CitaServicios(models.Model):
    ...
    servicio = models.ForeignKey('Servicios', models.DO_NOTHING, db_comment='Este campo es la llave primaria y forßnea de la tabla. Almacena el identificador del servicio prestado, haciendo referencia a la tabla SERVICIOS. Esto permite asociar los servicios especÝficos que se ofrecen en cada cita.')
...
class Clinicas(models.Model):
    ...
    usuario_propietario = models.ForeignKey('Usuarios', models.DO_NOTHING, db_column='usuario_propietario', db_comment='Este campo es una llave forßnea que hace referencia al usuario que es due±o o administrador de la clÝnica. Ayuda a relacionar la clÝnica con su propietario o administrador en el sistema.')
...
class ConsultaMascotas(models.Model):
    ...
    mascota = models.ForeignKey('Mascotas', models.DO_NOTHING, db_comment='Este campo es una llave forßnea que hace referencia a la mascota que fue consultada, vinculßndose a la tabla MASCOTAS. Permite identificar a quÚ mascota pertenece cada consulta.')
    ...
    diagnostico = models.CharField(max_length=100, blank=True, null=True, db_comment='Este campo proporciona el diagn¾stico obtenido durante la consulta. Es esencial para el registro mÚdico y permite documentar la salud de la mascota y cualquier tratamiento recomendado.')
...
class ConsultaSintomas(models.Model):
    ...
    consulta = models.ForeignKey(ConsultaMascotas, models.DO_NOTHING, db_comment='Este campo es una llave primaria y forßnea que almacena el identificador de la consulta donde se observ¾ el sÝntoma. Hace referencia a la tabla CONSULTA_MASCOTAS, asociando cada sÝntoma observado con su consulta correspondiente.')
...
class ConsultaVacunas(models.Model):
    vacuna = models.OneToOneField('Vacunas', models.DO_NOTHING, primary_key=True, db_comment='Este campo es una llave primaria y forßnea que almacena el identificador de la vacuna administrada. Hace referencia a la tabla VACUNAS, permitiendo identificar quÚ vacuna se administr¾ durante la consulta.')  # The composite primary key (vacuna_id, consulta_id) found, that is not supported. The first column is selected.
...
class Especialidades(models.Model):
    especialidad_id = models.FloatField(primary_key=True, db_comment='Este campo es la llave primaria de la tabla. Almacena un identificador ·nico para cada especialidad. DeberÝa ser un valor ·nico que permite distinguir cada especialidad de manera clara.')
    nombre = models.CharField(max_length=30, db_comment='Este campo almacena el nombre de la especialidad (por ejemplo, "DermatologÝa", "CirugÝa", "OdontologÝa"). Es un campo importante para identificar la especialidad de manera comprensible.')
...
class HistorialServicios(models.Model):
    servicio = models.OneToOneField('Servicios', models.DO_NOTHING, primary_key=True, db_comment='Este campo es la llave primaria y forßnea de la tabla. Almacena el identificador del servicio prestado, haciendo referencia a la tabla SERVICIOS. Permite vincular el historial de servicios con el servicio especÝfico ofrecido.')  # The composite primary key (servicio_id, fecha) found, that is not supported. The first column is selected.
    ...
    descripcion = models.CharField(max_length=50, blank=True, null=True, db_comment='Este campo proporciona una descripci¾n detallada del servicio que se realiz¾. Ayuda a registrar informaci¾n especÝfica sobre la atenci¾n que recibi¾ la mascota en cada ocasi¾n.')
...
class HorariosVeterinarios(models.Model):
    ...
    usuario_veterinario = models.ForeignKey('Usuarios', models.DO_NOTHING, db_column='usuario_veterinario')
...
class MetodosDePago(models.Model):
    metodo_pago_id = models.FloatField(primary_key=True, db_comment='Este campo es la llave primaria de la tabla. Almacena un identificador ·nico para cada mÚtodo de pago. Este valor debe ser ·nico y permite distinguir cada mÚtodo de manera clara.')
    usuario_cliente = models.ForeignKey('Usuarios', models.DO_NOTHING, db_column='usuario_cliente', db_comment='Este campo es una llave forßnea que hace referencia al usuario cliente que posee el mÚtodo de pago. Permite vincular el mÚtodo de pago con el cliente correspondiente en el sistema.')
    tipo_pago = models.CharField(max_length=20, db_comment='Este campo almacena el tipo de pago (por ejemplo, "Tarjeta de crÚdito", "Tarjeta de dÚbito", "Transferencia bancaria"). Esto ayuda a clasificar los mÚtodos de pago disponibles.')
    ...
    nombre_titular = models.CharField(max_length=40, db_comment='Este campo almacena el nombre del titular de la tarjeta. Esto es importante para verificar que el mÚtodo de pago estÚ registrado correctamente a nombre del cliente.')
    ...
    estado = models.CharField(max_length=20, db_comment='Este campo almacena el estado del mÚtodo de pago (por ejemplo, "Activo", "Inactivo"). Esto permite gestionar quÚ mÚtodos de pago estßn disponibles para su uso.')
...
class Pagos(models.Model):
    ...
    metodo_pago = models.ForeignKey(MetodosDePago, models.DO_NOTHING, db_comment='Este campo es la llave primaria y forßnea de la tabla. Almacena el identificador del mÚtodo de pago utilizado, haciendo referencia a la tabla METODOS_DE_PAGO. Esto permite identificar c¾mo se realiz¾ cada pago (por ejemplo, tarjeta de crÚdito, efectivo).')
...
class Roles(models.Model):
    ...
    abreviacion = models.CharField(max_length=20, db_comment='Este campo guarda una versi¾n abreviada o un acr¾nimo del nombre del rol. Por ejemplo, "ADM" para "Administrador" o "VET" para "Veterinario". TambiÚn es un campo obligatorio y proporciona una forma rßpida de referirse al rol en sistemas o interfaces donde el espacio es limitado.')
...
class Sintomas(models.Model):
    sintoma_id = models.FloatField(primary_key=True, db_comment='Este campo es la llave primaria de la tabla. Almacena un identificador ·nico para cada sÝntoma, lo que permite distinguir claramente entre diferentes sÝntomas.')
    descripcion = models.CharField(max_length=100, blank=True, null=True, db_comment='Este campo proporciona una descripci¾n del sÝntoma. Es importante para ayudar a los veterinarios y al personal a reconocer y evaluar adecuadamente los sÝntomas que pueden presentar las mascotas.')
    nombre = models.CharField(max_length=30, db_comment='Este campo almacena el nombre del sÝntoma. Es fundamental para identificar rßpidamente el sÝntoma en la base de datos y en las interacciones con los clientes.')
...
class Tratamientos(models.Model):
    ...
    descripcion = models.CharField(max_length=100, blank=True, null=True, db_comment='Este campo proporciona una descripci¾n detallada del tratamiento. Es importante para que los veterinarios y el personal de la clÝnica comprendan el prop¾sito y la naturaleza del tratamiento.')
...
class Usuarios(models.Model):
    ...
    cedula = models.CharField(max_length=30, db_comment='Este almacena el n·mero de cÚdula del usuario.')
    nombre = models.CharField(max_length=50, db_comment='Este campo almacena el nombre del usuario. Es ·til para identificar al usuario dentro del sistema.')
    ...
    especialidad = models.ForeignKey(Especialidades, models.DO_NOTHING, blank=True, null=True, db_comment='Este campo es una llave forßnea que hace referencia a la especialidad del usuario, si aplica. Permite vincular al usuario con una especialidad especÝfica en la clÝnica.')
