...
const ModifyMyPets = ({ open, handleClose, petData, onSuccess }) => {
  ...
  useEffect(() => {
    if (petData) {
      setFormData({
        ...
        fecha_nacimiento:
          dayjs(petData.FECHA_NACIMIENTO).format("YYYY-MM-DD") || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        onSuccess("Mascota modificada exitosamente", "success");
        handleClose();
      }
    } catch (error) {
      const errorMessage =
